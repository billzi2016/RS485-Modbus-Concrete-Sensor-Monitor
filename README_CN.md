...
当前 WebUI 展示使用 mock 数据，用于演示混凝土传感器监控平台的页面效果、指标矩阵、趋势曲线和数据源切换能力。
...
系统面向大规模混凝土传感器部署设计，可以通过增加 Modbus 网关、采集 worker、Redis 容量和应用实例扩展到百万级传感器监控场景。

实际可承载规模主要取决于现场网络是否拥塞以及链路质量。生产部署时需要综合评估网关带宽、采样频率、丢包率、Redis 写入吞吐和前端查询负载。
